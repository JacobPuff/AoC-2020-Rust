...
pub fn day7() {
    ...
    for rule in rules {
        let bag_and_sub_bags: Vec<&str> = rule.split(" bags contain ").collect();
        let sub_bag_strs: Vec<&str> = Regex::new(r"\sbag(s)?[.,](\s)?").unwrap().split(bag_and_sub_bags[1]).collect();
        let mut sub_bags: Vec<SubBag> = Vec::new();

        for sub_bag in sub_bag_strs {
            if sub_bag != "no other" && sub_bag != "" {
                let amount_and_name: Vec<&str> = sub_bag.splitn(2, " ").collect();
                let name = amount_and_name[1];
                let parsed_amount = amount_and_name[0].parse::<i32>().unwrap();
                let new_sub_bag: SubBag = SubBag {
                    name: name,
                    amount: parsed_amount
                };
                sub_bags.push(new_sub_bag)
            }
        }

        let bag: Bag = Bag {
            name: bag_and_sub_bags[0],
            contains: sub_bags
        };
        rule_map.insert(bag.name, bag);
    }
...
fn contains_bag(rule_map: &HashMap<&str, Bag>, bag_name: &str, contains_bag_name: &str) -> bool {
    let bag: &Bag = &rule_map[bag_name];
    for sub_bag in &bag.contains {
        if sub_bag.name == contains_bag_name {
            return true
        }
        if contains_bag(rule_map, sub_bag.name, contains_bag_name) {
            return true
        }
    }
    return false
...
const DATA: &str = "vibrant salmon bags contain 1 vibrant gold bag, 2 wavy aqua bags, 1 dotted crimson bag.
