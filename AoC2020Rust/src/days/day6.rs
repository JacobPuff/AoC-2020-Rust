...
pub fn day6() {
    ...
    let mut part_one_groups_added = 0;
    let mut part_two_groups_added = 0;
    for group in groups {
        let mut answered_yes = HashMap::new();
        let group_members: Vec<&str> = group.split("\n").collect();
        for answers in &group_members {
            for c in answers.chars() {
                let mut new_val = 1;
                if answered_yes.contains_key(&c) {
                    new_val = answered_yes[&c] + 1
                }
                answered_yes.insert(c,new_val);
            }
        }
        // Part one
        part_one_groups_added += answered_yes.len();

        // Part two
        for (_question, answered_amount) in &answered_yes {
            if *answered_amount as usize == group_members.len() {
                part_two_groups_added += 1;
            }
        }
    }

    // Part one anyone answered yes 6799
    println!("Part one anyone answered yes {}", part_one_groups_added);
    // Part two everyone answered yes 3354
    println!("Part two everyone answered yes {}", part_two_groups_added);
}

const DATA: &str = "fekdcbayqxnwvh
fycwqktvxandeb
kqbafvcxyewrdn
akwqcvenxfydbs
ewbaxdcvnkyfq
