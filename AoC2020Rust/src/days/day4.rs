pub fn day4() {
    let mut part_one_num_of_valid_passports = 0;
    let mut part_two_num_of_valid_passports = 0;
    ...
    for passport in passports {
        ...
        for key_value in values {
            ...
            // Part two
            if part_two_checks(field, value) == false {
                part_two_valid = false;
            }
        }
        if (byr && iyr && eyr && hgt && hcl && ecl && pid) == true {
            part_one_valid = true;
            part_one_num_of_valid_passports += 1;
        }

        if part_two_valid && part_one_valid {
            part_two_num_of_valid_passports += 1;
        }
    }
    // Part one num valid passports 222
    println!("Part one num valid passports {}", part_one_num_of_valid_passports);
    // Part two num valid passports 140
    println!("Part two num valid passports {}", part_two_num_of_valid_passports);
}

fn part_two_checks(field: &str, value: &str) -> bool {
    if field == "byr" {
        if value.len() != 4 {
            return false
        }
    ...
    if field == "hgt" {
        // Must contain cm or in
        if value.len() <= 2 {
            return false
        }

        let format = &value[value.len()-2..];
        let size_str = &value[..value.len()-2];
    ...
    if field == "hcl" {
        ...
        if !hex_value.chars().all(|c| "abcdef0123456789".contains(c)) {
            return false
        }
    }
